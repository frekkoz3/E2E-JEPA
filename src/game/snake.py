...
if __name__ == "__main__":
    ...
    for _ in range(10):
        # action = headless_env.action_space.sample()
        state_tensor = torch.tensor(img_obs, dtype=torch.float32)
        state_tensor = state_tensor.reshape(1, 1, -1)
        action = policy.get_action(state_tensor, greedy = True)
        img_obs, rew, done, trunc, info = headless_env.step(action)
